    ...
    <div class="container pd-x-0">
        <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
            <div>
                ...
                <h4 class="mg-b-0 tx-spacing--1">Edit Career</h4>
            </div>
        </div>

        <form id="updateForm" method="POST" action="{{ route('careers.update', $career->id) }}" enctype="multipart/form-data">
            <div class="row row-sm">
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input name="name" id="name" value="{{ old('name', $career->name) }}" required type="text" class="form-control @error('name') is-invalid @enderror">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="selectpicker mg-b-5" required data-style="btn btn-outline-light btn-md btn-block tx-left" title="Select career category" data-width="100%">
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" @if(old('category_id', $career->category_id) == $category->id) selected @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input name="vacant_pos" id="vacant_pos" value="{{ old('vacant_pos',$career->vacant_pos) }}" required type="number" class="form-control @error('vacant_pos') is-invalid @enderror">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control @error('start_date') is-invalid @enderror" name="start_date" value="{{ old('start_date', $career->start_date) }}">
                        @hasError(['inputName' => 'start_date'])
                        @endhasError
                    </div>
                    <div class="form-group">
                        <label class="d-block">End Date *</label>
                        <input type="date" class="form-control @error('end_date') is-invalid @enderror" name="end_date" value="{{ old('end_date', $career->end_date) }}">
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('teaser') is-invalid @enderror" name="teaser" rows="4" required>{{ old('teaser', $career->teaser) }}</textarea>
                        @error('teaser')
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-switch @error('is_active') is-invalid @enderror">
                            <input type="checkbox" class="custom-control-input" name="is_active" {{ (old("is_active", $career->is_active) ? "checked":"") }} id="customSwitch1">
                            <label class="custom-control-label" id="label_visibility" for="customSwitch1">{{ (old("is_active", $career->is_active) ? "Published" : "Private") }}</label>
                ...
                <div class="col-lg-12 mg-t-30">
                    <input class="btn btn-primary btn-sm btn-uppercase" type="submit" value="Update Career">
